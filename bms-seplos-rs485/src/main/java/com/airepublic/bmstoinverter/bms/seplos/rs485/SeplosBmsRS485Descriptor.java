...
/**
 * The {@link BMSDescriptor} for the Seplos BMS using the RS485 protocol.
 */
public class SeplosBmsRS485Descriptor implements BMSDescriptor {
    @Override
    ...
    public Class<? extends BMS> getBMSClass() {
        return SeplosBmsRS485Processor.class;
    }


    @Override
    public Port createPort(final BMSConfig config) {
        final Port port = new JSerialCommPort(config.getPortLocator(), config.getBaudRate(), 8, 1, SerialPort.NO_PARITY, new byte[] { (byte) 126 }, FrameDefinition.create("SOOAAOOOOLLLLDCCCCO"));
        return port;
    }
