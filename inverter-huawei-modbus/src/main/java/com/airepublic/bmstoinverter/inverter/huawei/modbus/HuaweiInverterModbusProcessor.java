...
public class HuaweiInverterModbusProcessor extends Inverter {
    ...
    private int[] getAlarms(final BatteryPack aggregatedPack) {
        final BitSet bits39014 = new BitSet(16);
        final BitSet bits39015 = new BitSet(16);
        final BitSet bits39016 = new BitSet(16);

        bits39014.set(13, aggregatedPack.alarms.get(Alarm.FAILURE_COMMUNICATION_INTERNAL) == AlarmLevel.ALARM);
        bits39014.set(15, aggregatedPack.alarms.get(Alarm.FAILURE_OTHER) == AlarmLevel.ALARM);
        bits39016.set(0, aggregatedPack.alarms.get(Alarm.PACK_VOLTAGE_HIGH) == AlarmLevel.ALARM);
        bits39016.set(15, aggregatedPack.alarms.get(Alarm.PACK_VOLTAGE_LOW) == AlarmLevel.ALARM);
        bits39016.set(12, aggregatedPack.alarms.get(Alarm.PACK_CURRENT_HIGH) == AlarmLevel.ALARM);
        bits39015.set(1, aggregatedPack.alarms.get(Alarm.FAILURE_SHORT_CIRCUIT_PROTECTION) == AlarmLevel.ALARM);
        bits39016.set(13, aggregatedPack.alarms.get(Alarm.PACK_TEMPERATURE_HIGH) == AlarmLevel.ALARM);
        bits39016.set(14, aggregatedPack.alarms.get(Alarm.PACK_TEMPERATURE_LOW) == AlarmLevel.ALARM);
        bits39015.set(15, aggregatedPack.alarms.get(Alarm.TEMPERATURE_SENSOR_DIFFERENCE_HIGH) == AlarmLevel.ALARM);

        return new int[] { (int) bits39014.toLongArray()[0], (int) bits39015.toLongArray()[0], (int) bits39016.toLongArray()[0] };
    }
